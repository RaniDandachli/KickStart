-- Mirror of supabase/migrations/00035_h2h_quick_match_allowed_tiers.sql

alter table public.h2h_queue_entries
  add column if not exists quick_match_allowed_entry_cents bigint[];

comment on column public.h2h_queue_entries.quick_match_allowed_entry_cents is
  'Quick Match wildcard only: which contest access tiers (entry wallet cents) this searcher accepts; null/legacy treated as free-only {0}.';

drop function if exists public.h2h_enqueue_quick_match(text, bigint);
...
comment on function public.h2h_enqueue_quick_match(text, bigint, bigint[]) is
  'Quick Match: specific waiters (tier must be allowed), WW vs WW (max common tier), pending session pickup, then wildcard wait with tier allowlist.';
