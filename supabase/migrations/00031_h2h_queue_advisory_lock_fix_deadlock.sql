-- BUGFIX: Concurrent polls from two players caused a deadlock:
--   Tx A locks B's queue row, Tx B locks A's queue row, then both need each other's locks → deadlock.
--   The killed transaction rolled back the session INSERT, so zero match_sessions were ever created.
-- FIX: pg_advisory_xact_lock serialises queue operations per mode — one transaction at a time, no cross-locks.
...
begin
  ...
  -- Serialise all queue mutations for this mode so two concurrent polls cannot deadlock.
  ...
  -- If another player already paired us, pick up the existing session.
  select ms.id,
  ...
  -- Path 1: match a wildcard (__quick_match__) waiter whose budget covers this entry.
  ...
  -- No partner found — maintain or insert a waiting row for this tier.
...
begin
  ...
  -- Pick up session created when another player matched us.
  select ms.id,
         case when ms.player_a_id = v_me then ms.player_b_id else ms.player_a_id end
    into v_sid, v_partner_user
  from public.match_sessions ms
  ...
  -- Match with any affordable specific-game waiter.
  select
    q.id,
    q.user_id,
...
comment on function public.h2h_enqueue_or_match(text, text, bigint, bigint) is
  'Join H2H queue or match; advisory-locked per mode to prevent deadlocks between concurrent polls.';

comment on function public.h2h_enqueue_quick_match(text, bigint) is
  'Quick Match: pair with affordable waiter; advisory-locked per mode to prevent deadlocks.';
