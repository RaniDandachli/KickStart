-- Void Runner: prize runs + H2H (game_key void-runner → minigame_scores.game_type void_runner).
...
begin
  ...
  if p_game_type is null
    or p_game_type !~ '^(tap_dash|tile_clash|ball_run|neon_pool|stacker|dash_duel|turbo_arena|neon_dance|neon_grid|neon_ship|void_runner)$'
  then
...
comment on function public.h2h_tap_dash_scores_for_match(uuid) is
  'H2H skill contest: latest validated minigame_scores for both players (includes void-runner → void_runner).';
