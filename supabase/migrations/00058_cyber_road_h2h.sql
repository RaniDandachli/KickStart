-- Mirror of sql/migrations/00058_cyber_road_h2h.sql

-- Cyber Road — H2H score polling (game_key cyber-road ↔ minigame_scores.game_type cyber_road).
...
begin
  ...
  if v_gk = '' or v_gk = 'tap-dash' then
    ...
    v_gt := 'neon_ship';
  elsif v_gk = 'shape-dash' then
    v_gt := 'shape_dash';
  elsif v_gk = 'cyber-road' then
    v_gt := 'cyber_road';
  else
...
comment on function public.h2h_tap_dash_scores_for_match(uuid) is
  'H2H skill contest: latest validated minigame_scores for both players (includes cyber-road → cyber_road).';
