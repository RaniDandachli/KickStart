-- Mirror of supabase/migrations/00048_neon_grid_minigame.sql

-- Neon Grid: prize runs + H2H score polling (game_key neon-grid → minigame_scores.game_type neon_grid).
...
begin
  ...
  if p_game_type is null
    or p_game_type !~ '^(tap_dash|tile_clash|ball_run|neon_pool|stacker|dash_duel|turbo_arena|neon_dance|neon_grid)$'
  then
...
comment on function public.h2h_tap_dash_scores_for_match(uuid) is
  'H2H skill contest: latest validated minigame_scores for both players (includes neon-grid → neon_grid).';
