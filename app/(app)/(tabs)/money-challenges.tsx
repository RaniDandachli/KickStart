...
/** Tab root: score targets · free daily tiers + wallet-entry tiers (Tap Dash first). */
    ...
    async (c: (typeof MONEY_CHALLENGES)[number]) => {
      ...
      if (c.kind === 'paid') {
        ...
        if (!r.ok) {
          Alert.alert(
            'Wallet',
            r.error ?? 'Could not unlock today’s tier. Add funds if needed.',
          );
          return;
        }
        await refresh();
      ...
      const ret = encodeURIComponent(String(moneyChallengesHref()));
      router.push(
        `/(app)/(tabs)/play/minigames/tap-dash?challengeId=${q.challengeId}&targetScore=${q.targetScore}&prizeLabel=${q.prizeLabel}&returnHref=${ret}` as never,
      );
    },
  ...
  return (
    <Screen scroll>
      ...
      <Text style={styles.sub}>
        Hit showcase score targets · free daily tiers (10 tries) or wallet boosts with bigger showcases —
        prizes subject to eligibility & verification
      </Text>
