...
import { getMoneyChallengeById, MONEY_CHALLENGES, toSoloChallengeBundle } from '@/lib/moneyChallenges';
...
function buildNavigateParams(bundle: SoloChallengeBundle): { challengeId: string; targetScore: string; prizeLabel: string } {
  return {
...
/** Hub: score targets · free daily tiers + wallet-entry tiers (Tap Dash first). */
export default function MoneyChallengesScreen() {
  const router = useRouter();
  const uid = useAuthStore((s) => s.user?.id);
  const qc = useQueryClient();
  const [tries, setTries] = useState<Record<string, number>>({});
  const dayKey = useMemo(() => todayYmdLocal(), []);
  ...
  const startChallenge = useCallback(
    async (c: (typeof MONEY_CHALLENGES)[number]) => {
      ...
      if (c.kind === 'paid') {
        if (!ENABLE_BACKEND) {
          Alert.alert('Unavailable', 'Wallet tiers need the live API (EXPO_PUBLIC_ENABLE_BACKEND=true).');
          return;
        }
        if (!uid) {
          Alert.alert('Sign in required', 'Paid Money Challenges debit your cash wallet once per day.');
          return;
        }
        const r = await enterMoneyChallengeWallet(c.id, dayKey);
        if (!r.ok) {
          Alert.alert('Wallet', r.error ?? 'Could not unlock today’s tier. Add funds if needed.');
          return;
        }
        await refresh();
      ...
      router.push(
        `/(app)/(tabs)/play/minigames/tap-dash?challengeId=${q.challengeId}&targetScore=${q.targetScore}&prizeLabel=${q.prizeLabel}` as never,
      );
    },
    [dayKey, qc, refresh, router, uid],
  );
  ...
  return (
    <Screen scroll>
      ...
      <Text style={[styles.title, { fontFamily: runitFont.black }, runitTextGlowPink]}>MONEY CHALLENGES</Text>
      <Text style={styles.sub}>
        Hit showcase score targets · free daily tiers (10 tries) or wallet boosts with bigger showcases — prizes subject to eligibility & verification
      </Text>
      ...
      {MONEY_CHALLENGES.map((c) => {
        ...
        return (
          <Pressable key={c.id} onPress={() => void startChallenge(c)} style={({ pressed }) => [styles.cardWrap, pressed && { opacity: 0.93 }]}>
            <LinearGradient
            ...
            >
              <View style={styles.cardInner}>
                ...
                <Text style={styles.prize}>
                  ...
                  {c.entryFeeWalletCents != null ? ` · $${(c.entryFeeWalletCents / 100).toFixed(2)} once per day to unlock attempts` : ''}
                </Text>
                <Text style={styles.tries}>
                  Remaining tries: {remaining}/{cap}
                </Text>
                <View style={styles.footer}>
                  <Text style={styles.link}>{c.kind === 'paid' ? 'Unlock & chase target' : 'Play challenge'}</Text>
...
const styles = StyleSheet.create({
  ...
  backTxt: { color: '#FFD700', fontSize: 15 },
  title: { fontSize: 26, letterSpacing: 1, marginBottom: 8, color: '#fff' },
  ...
  rowTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 8 },
  cardName: { flex: 1, color: '#fff', fontSize: 17 },
  pill: { backgroundColor: 'rgba(236,72,153,0.35)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 },
