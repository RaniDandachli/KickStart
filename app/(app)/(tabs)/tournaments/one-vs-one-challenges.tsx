...
/** Tap Dash skill targets — async 1v1: your run posts a score; when others play, higher score wins (not a live match). */
export default function OneVsOneChallengesScreen() {
  ...
  const startChallenge = useCallback(
    async (c: (typeof MONEY_CHALLENGES)[number]) => {
      ...
      if (c.kind === 'paid') {
        ...
        if (!uid) {
          Alert.alert('Sign in required', 'Paid tiers debit your cash wallet once per day to unlock attempts.');
          return;
        }
        ...
        if (!r.ok) {
          ...
          const msg =
            code === 'insufficient_wallet'
              ? 'Not enough cash in your wallet for this tier. Add funds on your profile, then try again.'
      ...
      const ret = encodeURIComponent(String(oneVsOneChallengesHref()));
      router.push(
  ...
  return (
    <Screen scroll>
      <Text style={[styles.title, { fontFamily: runitFont.black }, runitTextGlowPink]}>1V1 CHALLENGES</Text>
      <Text style={styles.sectionLabel}>ASYNC SKILL MATCHUPS</Text>
      <Text style={styles.sub}>
        You play for a score on your schedule — not a live head-to-head. When another player takes the same
        challenge, their run is stacked against yours: better score wins. Play sharp and beat the line.
      </Text>
