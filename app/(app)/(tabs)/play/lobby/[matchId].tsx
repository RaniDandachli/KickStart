...
export default function PreMatchLobbyScreen() {
  ...
  return (
    <Screen>
      ...
      <Card className="mb-4">
        <Text className="text-sm text-white/60">Session</Text>
        <Text className="text-lg font-mono text-neon-lime">{matchId}</Text>
      </Card>
      <View className="flex-row gap-3">
        <PlayerTile label="You" sub={self} />
        <PlayerTile label="Opponent" sub="Resolved from Supabase later" />
      </View>
      <Text className="my-4 text-sm text-white/50">
...
function PlayerTile({ label, sub }: { label: string; sub: string }) {
  return (
    <View className="flex-1 rounded-2xl border border-white/10 bg-ink-800 p-3">
      <Text className="text-xs uppercase text-white/50">{label}</Text>
      <Text className="text-base font-semibold text-white">{sub}</Text>
    </View>
  );
}
