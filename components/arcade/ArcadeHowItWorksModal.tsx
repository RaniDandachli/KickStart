...
const ARCADE_STEPS: StepDef[] = [
  {
    title: 'Pick a minigame',
    body: 'Tap any game in Hot Games — same titles as on Home, but here you play solo against the course and your own best score.',
    icon: 'game-controller-outline',
    frameColors: ['rgba(124,58,237,0.55)', 'rgba(30,27,75,0.95)'],
    iconTint: '#a5b4fc',
  },
  {
    title: 'Practice or prize run',
    body: 'Practice is free. Prize runs spend Arcade Credits (shown in your balance). Beat the challenge to earn redeem tickets toward the Prizes tab.',
    icon: 'flash-outline',
    frameColors: ['rgba(236,72,153,0.45)', 'rgba(76,29,149,0.9)'],
    iconTint: '#f9a8d4',
  },
  {
    title: 'Credits & tickets',
    body: 'Arcade Credits power prize runs. Tickets stack when you win prize runs — save them for gift cards, gear, or other rewards in Prizes.',
    icon: 'ticket-outline',
    frameColors: ['rgba(255,215,0,0.35)', 'rgba(59,7,100,0.92)'],
    iconTint: '#7dd3fc',
  },
  {
    title: 'Head-to-head lives on Home',
    body: 'Real-time 1v1 skill contests with tier prizes are on the Home tab. Use Arcade to warm up and earn tickets — then stake a tier when you’re ready.',
    icon: 'home-outline',
...
export function ArcadeHowItWorksModal({ visible, onClose }: Props) {
  ...
  const last = stepIndex >= ARCADE_STEPS.length - 1;
          ...
          <View style={styles.headerRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.stepKicker, runitTextGlowCyan]}>
                Arcade · step {stepIndex + 1} of {ARCADE_STEPS.length}
              </Text>
            </View>
            <Pressable
              onPress={onClose}
          ...
          <View style={styles.footer}>
            ...
            <View style={styles.navBtns}>
              ...
              {last ? (
                ...
                <Pressable
                  onPress={() => setStepIndex((s) => Math.min(ARCADE_STEPS.length - 1, s + 1))}
