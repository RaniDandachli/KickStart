...
import { SidebarBrandedSocialRow, type SidebarSocialKey } from '@/components/web/SidebarBrandedSocialRow';
...
const ITEMS: SidebarItem[] = [
  ...
  { key: 'tournaments', label: 'Events', href: '/(app)/(tabs)/tournaments', icon: 'calendar-outline' },
  { key: 'play', label: 'Arcade', href: '/(app)/(tabs)/play', icon: 'game-controller' },
  { key: 'prizes', label: 'Prizes', href: '/(app)/(tabs)/prizes', icon: 'medal-outline' },
  { key: 'profile', label: 'You', href: '/(app)/(tabs)/profile', icon: 'person' },
];

const SOCIAL_KEYS_SIDEBAR: SidebarSocialKey[] = ['discord', 'instagram', 'x'];
        ...
        {ITEMS.map((it) => {
          ...
          return (
            <View key={it.key}>
              ...
              {it.key === 'profile' ? (
                <SidebarBrandedSocialRow
                  items={SOCIAL_KEYS_SIDEBAR.map((key) => ({
                    key,
                    url: ARCADE_SOCIAL_URLS[key] ?? '',
                  }))}
                  onOpen={openSocial}
                />
              ) : null}
