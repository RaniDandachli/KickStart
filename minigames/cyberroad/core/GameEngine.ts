...
export default class Engine {
  ...
  /** Head-to-head: final row score, wall-clock run duration, swipe/tap count (matches updateScore). */
  getH2hRunStats = () => {
    const score = Math.max(Math.floor(this._hero.position.z) - 8, 0);
    const durationMs =
      this.runStartMs != null ? Math.max(0, Date.now() - this.runStartMs) : 0;
    return { score, durationMs, taps: this.inputMoveCount };
  };
