...
export function GameplayPlaceholder({ session, onFinish, onPauseToggle }: GameplayPlaceholderProps) {
  ...
  return (
    <View className="gap-3">
      <Card>
        <Text className="mb-1 text-xs uppercase text-white/50">Prototype arena</Text>
        <Text className="text-2xl font-black text-neon-lime">KickClash Engine Stub</Text>
        <Text className="mt-1 text-sm text-white/60">
          Timer + score only. TODO: inputs, ball physics, networking reconciliation.
        </Text>
      </Card>
      <Card className="items-center">
        <Text className="text-5xl font-black text-white">{remaining}s</Text>
        <Text className="text-white/50">Match id · {session.id}</Text>
      </Card>
      <View className="flex-row justify-between gap-3">
        <Card className="flex-1 items-center">
          <Text className="text-xs text-white/50">You</Text>
          <Text className="text-4xl font-bold text-neon-cyan">{scoreSelf}</Text>
          <AppButton title="+Goal" variant="secondary" onPress={() => setScoreSelf((s) => s + 1)} />
        </Card>
        <Card className="flex-1 items-center">
          <Text className="text-xs text-white/50">Opp</Text>
          <Text className="text-4xl font-bold text-neon-magenta">{scoreOpponent}</Text>
